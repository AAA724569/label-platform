...
def _render_loc_detail(df_f: pd.DataFrame, loc_row):
    ...
    times        = [str(t)[:10] for t in loc_rows_all["collection_time"].dropna().unique() if t]
    collect_str  = "、".join(times[:3]) if times else "—"
...
def page_dashboard():
    ...
    with st.expander("⚙️ 高级筛选", expanded=False):
        ...
        _adv4, _adv5, _adv6, _adv7 = st.columns(4)

        sub_opts  = sorted(df["top_road_subcategory"].dropna().unique())
        road_opts = _tag_vals(df, "一、道路静态环境", "1.6 道路交叉", "交叉类型")
        lane_opts = _tag_vals(df, "一、道路静态环境", "1.4 包含车道特征", "最宽车道数量")
        ...
        with _adv1:
            sel_sub  = st.multiselect("道路子类", sub_opts,  default=sub_opts,  key="adv_sub")
        with _adv2:
            sel_cross= st.multiselect("交叉类型", road_opts, default=road_opts, key="adv_cross")
        with _adv3:
            sel_lane = st.multiselect("车道数量", lane_opts, default=lane_opts, key="adv_lane")
        with _adv4:
            sel_surf = st.multiselect("路面类型", surf_opts, default=surf_opts, key="adv_surf")
        with _adv5:
            sel_sig  = st.multiselect("信号灯",   sig_opts,  default=sig_opts,  key="adv_sig")
        with _adv6:
            sel_wth  = st.multiselect("天气",      wth_opts,  default=wth_opts,  key="adv_wth")
        with _adv7:
            sel_period = st.multiselect("采集时段", PERIOD_ORDER, default=PERIOD_ORDER, key="adv_period")
    ...
    df_f = df[
        ...
        df["_tags"].apply(lambda d: _tag_match(d, "一、道路静态环境", "1.4 包含车道特征", "最宽车道数量", sel_lane)) &
...
.kpi-icon { font-size:1.5rem; margin-bottom:4px; position:relative; z-index:1; }
.kpi-num  { font-size:2.6rem; font-weight:800; line-height:1.05;
            letter-spacing:-1px; margin:2px 0; position:relative; z-index:1; }
.kpi-lbl  { font-size:0.72rem; font-weight:600; letter-spacing:0.8px;
            opacity:0.82; margin-top:2px; position:relative; z-index:1; }
.kpi-blue   { background:linear-gradient(135deg,#1a56a8 0%,#0d2d5e 100%);
              box-shadow:0 6px 22px rgba(13,45,94,0.42); }
.kpi-indigo { background:linear-gradient(135deg,#3040c8 0%,#1e2a9c 100%);
              box-shadow:0 6px 22px rgba(30,42,156,0.38); }
.kpi-teal   { background:linear-gradient(135deg,#0ea5c8 0%,#0b6fa0 100%);
              box-shadow:0 6px 22px rgba(14,165,200,0.36); }
.kpi-purple { background:linear-gradient(135deg,#6a30c8 0%,#4a1ea0 100%);
              box-shadow:0 6px 22px rgba(106,48,200,0.36); }
...
""", unsafe_allow_html=True)
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["🏷️ 地点标注", "📊 统计看板", "📸 图片地点", "➕ 新增地点"])
    with tab1:
        page_label()
    with tab2:
        page_dashboard()
